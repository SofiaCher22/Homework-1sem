...
def calculate(*args):
    try:
        value1 = float(input()) # используем геттер для объекта StringVal
        value2 = float(input())
        sign =input()# используем сеттер для объекта StringVal
        if sign=='+':
            print(value1+value2)
        elif sign=='*':
            print(value1*value2)
...
'''
meters = StringVar()
ttk.Label(mainframe, textvariable=value1, value2).grid(column=2, row=2, sticky=(W, E))
